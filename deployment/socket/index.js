module.exports = function(server) {
	...
	io.sockets.on('connection', function(socket) {
			...
			commits: [
				{
					...
					author: {
						...
						avatar: 'https://bitbucket.org/account/levonazizyan/avatar/32/'
					}
				},
				{
		...
		io.emit('push', example);
		example.uuid = example.uuid + '1';
		io.emit('push', example);
		example.uuid = example.uuid + '2';
		io.emit('push', example);
		example.uuid = example.uuid + '3';
		io.emit('push', example);
		io.emit('push', example);
		example.uuid = example.uuid + '1';
		io.emit('push', example);
